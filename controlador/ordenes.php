		...
		switch($objModulo->getAction()){
			case 'add':
				...
				if ($band and $_POST['id'] == ''){
					$sms = new TSMS;
					$sms->setDestino($obj->getTelefono());
					$datos = array();
					$datos['folio'] = $obj->getFolio();
					$datos['idOrden'] = $obj->getId();
					
					$sms->setMensajeByPlantilla("repositorio/sms/ordenNueva.txt", $datos);
					$bandSMS = $sms->send();
					
					$email = new TMail();
					$email->setTema("Orden registrada");
					$email->addDestino($obj->getCorreo());
					$email->setBodyHTML(utf8_decode($email->construyeMail(file_get_contents("repositorio/mail/registrada.html"), $datos)));
					$result = $email->send();
				}
				
				$smarty->assign("json", array("band" => $band, "folio" => $obj->getFolio(), "SMS" => $bandSMS));
			break;
			case 'del':
			...
			case 'adjudicar':
				...
				if ($band){
					...
					$datos['folio'] = $obj->getFolio();
					$datos['idOrden'] = $obj->getId();
					$datos['transportista.razonsocial'] = $transportista->getRazonSocial();
					
					$sms->setMensajeByPlantilla("repositorio/sms/postulo.txt", $datos);
					$bandSMS = $sms->send();
					
					$email = new TMail();
					$email->setTema("Un transportista está interesado en tu carga");
					$email->addDestino($obj->getCorreo());
					$email->setBodyHTML(utf8_decode($email->construyeMail(file_get_contents("repositorio/mail/postulo.html"), $datos)));
					$result = $email->send();
			...
			case 'asignar':
				...
				if($band){
					...
					$sms->setMensajeByPlantilla("repositorio/sms/asignar.txt", $datos);
					$bandSMS = $sms->send();
					
					$email = new TMail();
					$email->setTema("Asignaste la carga");
					$email->addDestino($obj->getCorreo());
					$email->setBodyHTML(utf8_decode($email->construyeMail(file_get_contents("repositorio/mail/asignar.html"), $datos)));
					$result = $email->send();
				}
			...
			case 'setEnRuta':
				...
				if($band){
					...
					$sms->setMensajeByPlantilla("repositorio/sms/enRuta.txt", $datos);
					$bandSMS = $sms->send();
					
					$email = new TMail();
					$email->setTema("Vamos en camino");
					$email->addDestino($obj->getCorreo());
					$email->setBodyHTML(utf8_decode($email->construyeMail(file_get_contents("repositorio/mail/enRuta.html"), $datos)));
					$result = $email->send();
				}
			...
			case 'terminar':
				...
				if (band){
					...
					$sms->setMensajeByPlantilla("repositorio/sms/finalizado.txt", $datos);
					$bandSMS = $sms->send();
					
					$email = new TMail();
					$email->setTema("Hemos llegado");
					$email->addDestino($obj->getCorreo());
					$email->setBodyHTML(($email->construyeMail(file_get_contents("repositorio/mail/finalizado.html"), $datos)));
					$result = $email->send();
				}
